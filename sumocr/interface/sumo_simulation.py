...
from sumocr.interface.sumo_simulation_backend import (
    HeadlessSumoSimulationBackend,
    SumoSimulationBackend,
    SumoSimulationBackendConfiguration,
)
from sumocr.interface.sumo_simulation_interface import SumoSimulationInterface
...
from sumocr.scenario import AbstractScenarioWrapper, ScenarioWrapper
from sumocr.sumo_config import EGO_ID_START, DefaultConfig
from sumocr.sumo_config.pathConfig import SUMO_BINARY
...
_logger = logging.getLogger("sumocr")
...
class SumoSimulation:
    ...
    def __init__(self, simulation_backend: Optional[SumoSimulationBackend] = None):
        """
        :param simulation_backend: A pre-configured SumoSimulationBackend that will be used to interface with sumo
        """
        ...
        self._scenarios: Optional[ScenarioWrapper] = None
        ...
        # ego sync parameters
        self._lc_duration_max: int = 10
        self._lc_counter: int = (
            0  # Count how many steps are counter for the lane change
        )
        self._lc_inaction = (
            0  # Flag indicates that SUMO is performing a lane change for the ego
        )
        self.lateral_position_buffer = (
            dict()
    ...
    def scenarios(self, scenarios: ScenarioWrapper):
        ...
        self._max_lanelet_network_id = max(
            max_pp, max_lanelet_network_id(scenarios.lanelet_network)
        )
    ...
    def initialize(
        ...
        """
        # REFACTOR: if conf is None it transparently fallbacks to DefaultConfig
        if conf is not None:
            self.conf = conf
        ...
        if self._simulation_backend is None:
            # REFACTOR: This initialization should be performed by the caller, but that would require an API change
            ...
            self._simulation_backend = HeadlessSumoSimulationBackend(
                simulation_backend_config, self.scenarios.sumo_cfg_file, SUMO_BINARY
            )

        self._simulation_interface = SumoSimulationInterface(
            self._simulation_backend, self._id_mapper, self.scenarios.initial_scenario
        )

        # REFACTOR: this is a workaround to keep the same API
        self.simulationdomain = self._simulation_backend.simulation_domain
        self.vehicledomain = self._simulation_backend.vehicle_domain
        self.persondomain = self._simulation_backend.person_domain
        self.routedomain = self._simulation_backend.route_domain

        self._simulation_backend.start()

        # simulate until ego_time_start
        if (
            self.planning_problem_set is not None
            and len(self._find_ego_vehicles_in_rou_file()) == 0
        ):
            self.__presimulation_silent(
                max(0, self.conf.presimulation_steps - 1), fetch_obstacles=False
            )
            ...
            self.__presimulation_silent(
                self.conf.presimulation_steps, fetch_obstacles=True
            )
            self.initialized = True
    ...
    def init_ego_vehicles_from_planning_problem(
        ...
        if planning_problem_set is not None:
            ...
            for planning_problem in planning_problem_set.planning_problem_dict.values():
                ...
                sumo_id = IdDomain.EGO_VEHICLE.construct_sumo_id(new_id)
                cr_id = self._id_mapper.new_cr_id_from_sumo_id(
                    sumo_id=sumo_id, domain=IdDomain.EGO_VEHICLE
                )
                # Also add the EgoVehicle to the normal Vehcile IdDomain
                self._id_mapper.insert_mapping(sumo_id, cr_id, IdDomain.VEHICLE)
    ...
    def _add_ego_vehicle(self, ego_vehicle: EgoVehicle):
        ...
        if ego_vehicle.id in self._ego_vehicles:
            _logger.debug(
                f"Ego vehicle with id {ego_vehicle.id} already exists!", exc_info=True
            )
        else:
    ...
    def commonroad_scenario_at_time_step(
        self, time_step: int, add_ego=False, start_0=True
    ) -> Scenario:
        """
        ...
        self.cr_scenario.add_objects(
            self._get_cr_obstacles_at_time(time_step, add_ego=add_ego, start_0=start_0)
        )
    ...
    def _fetch_new_ids_from_sumo(self) -> Tuple[list, list]:
        """
        ...
        """
        new_vehicle_ids = (
            self._simulation_interface.vehicles.fetch_new_from_sumo_simulation()
        )
        for vehicle_id in new_vehicle_ids:
            vehicle_class = self.vehicledomain.getVehicleClass(str(vehicle_id))
            obstacle_type = sumo_vehicle_class_to_cr_obstacle_type(vehicle_class)
            self._set_veh_params(self.vehicledomain, vehicle_id, obstacle_type)

        new_pedestrian_ids = (
            self._simulation_interface.pedestrians.fetch_new_from_sumo_simulation()
        )
        for pedestrian_id in new_pedestrian_ids:
            vehicle_class = self.persondomain.getTypeID(pedestrian_id).split("@")[0]
            obstacle_type = sumo_vehicle_class_to_cr_obstacle_type(vehicle_class)

            self._set_veh_params(self.persondomain, pedestrian_id, obstacle_type)

        return new_vehicle_ids, new_pedestrian_ids

    def sync_obstacle_to_sumo_simulation(self, obstacle: DynamicObstacle):
        if (
            obstacle.obstacle_type == ObstacleType.CAR
            or obstacle.obstacle_type == ObstacleType.BICYCLE
        ):
            self._simulation_interface.vehicles.sync_to_sumo_simulation(
                obstacle, self._current_time_step
            )
        elif obstacle.obstacle_type == ObstacleType.PEDESTRIAN:
            self._simulation_interface.pedestrians.sync_to_sumo_simulation(
                obstacle, self._current_time_step
            )
        else:
            raise NotImplementedError()

    def sync_traffic_light_to_sumo_simulation(self, traffic_light):
        self._simulation_interface.traffic_lights.sync_to_sumo_simulation(
            traffic_light, self._current_time_step
        )
    ...
    def __presimulation_silent(self, pre_simulation_steps: int, fetch_obstacles=True):
        ...
        ), f"ego_time_start={self.conf.presimulation_steps} must be >0"
        ...
        for i in range(pre_simulation_steps * self.delta_steps):
            self._simulation_backend.simulation_step()
            self._fetch_new_ids_from_sumo()

        self._silent = False
        if fetch_obstacles:
            self._fetch_sumo_vehicles(
                self.current_time_step, self.vehicledomain.getIDList()
            )
            self._fetch_sumo_pedestrians(
                self.current_time_step, self.persondomain.getIDList()
            )

    def _subscribe_new(self, new_ids: List[SumoId], domain):
        """
    ...
    def _get_subscription_results(self, domain) -> Dict[str, Dict[int, any]]:
        """
    ...
    def _fetch_sumo_vehicles(self, time_step: int, new_ids: List[SumoId]):
        """
        Gets and stores all vehicle states from SUMO. Initializes ego vehicles when they enter simulation.

        """
        self._subscribe_new(new_ids, self.vehicledomain)
        vehicle_states = self._get_subscription_results(self.vehicledomain)
        ...
        for veh_id, vehicle_state in vehicle_states.items():
            state = self._extract_current_state(
                self.vehicledomain, veh_id, vehicle_state, IdDomain.VEHICLE
            )
            if state is None:
                continue
            ...
            if veh_id in new_ids:
                if (
                    veh_id.startswith(IdDomain.EGO_VEHICLE.sumo_prefix())
                    and not self._silent
                ):
                    # new ego vehicle
                    cr_id = self._id_mapper.sumo2cr(veh_id, IdDomain.EGO_VEHICLE)
                    ...
                    if self.planning_problem_set is not None:
                        ...
                        planning_problem = list(
                            self.scenarios.planning_problem_set.planning_problem_dict.values()
                        )[0]
                    else:
                    ...
                    cr_id = self._id_mapper.sumo2cr(veh_id, IdDomain.OBSTACLE_VEHICLE)
                    vehicle_class = self.vehicledomain.getVehicleClass(str(veh_id))
                    obstacle_type = sumo_vehicle_class_to_cr_obstacle_type(
                        vehicle_class
                    )

                    self.obstacle_types[cr_id] = obstacle_type
                    self.obstacle_shapes[cr_id] = self._get_veh_shape(
                        self.vehicledomain, veh_id
                    )
                    self.obstacle_states[time_step][cr_id] = state
            elif self._id_mapper.has_sumo2cr(veh_id, IdDomain.OBSTACLE_VEHICLE):
                # get obstacle vehicle state
                cr_id = self._id_mapper.sumo2cr(veh_id, IdDomain.OBSTACLE_VEHICLE)
                self.obstacle_states[time_step][cr_id] = state
            elif not self._silent and not self._id_mapper.has_sumo2cr(
                veh_id, IdDomain.EGO_VEHICLE
            ):
                raise NotImplementedError()

            # read signal state
            if not self._silent:
                signal_states = get_signal_state(
                    vehicle_state[traci.constants.VAR_SIGNALS], self.current_time_step
                )
                key = (
                    self._id_mapper.sumo2cr(veh_id, IdDomain.OBSTACLE_VEHICLE)
                    if self._id_mapper.has_sumo2cr(veh_id, IdDomain.OBSTACLE_VEHICLE)
                    else self._id_mapper.sumo2cr(veh_id, IdDomain.EGO_VEHICLE)
                )
            ...
            if (
                not self._silent
                and self.dummy_ego_simulation
                and self._id_mapper.has_sumo2cr(veh_id, IdDomain.EGO_VEHICLE)
            ):
                cr_id = self._id_mapper.sumo2cr(veh_id, IdDomain.EGO_VEHICLE)
                ego_veh = self.ego_vehicles[cr_id]
                ori = state.orientation
                state_list = []
                for t in range(0, self.conf.delta_steps):
                    state_tmp = copy.deepcopy(state)
                    state_tmp.position = state.position + (
                        t + 1
                    ) * state.velocity * self.dt * np.array(
                        [math.cos(ori), math.sin(ori)]
                    )
    ...
    def _fetch_sumo_pedestrians(self, time_step: int, new_ids: List[SumoId]):
        """
        Gets and stores all vehicle states from SUMO. Initializes ego vehicles when they enter simulation.

        """
        self._subscribe_new(new_ids, domain=self.persondomain)
        person_states = self._get_subscription_results(domain=self.persondomain)
        ...
        for ped_id, ped_state in person_states.items():
            state = self._extract_current_state(
                self.persondomain, ped_id, ped_state, IdDomain.PEDESTRIAN
            )
            if state is None:
                continue
            if ped_id in new_ids:
                ...
                cr_id = self._id_mapper.new_cr_id_from_sumo_id(
                    ped_id, IdDomain.PEDESTRIAN
                )
                vehicle_class = self.persondomain.getTypeID(ped_id).split("@")[0]
                obstacle_type = sumo_vehicle_class_to_cr_obstacle_type(vehicle_class)

                shape = self._set_veh_params(self.persondomain, ped_id, obstacle_type)

                self.obstacle_types[cr_id] = obstacle_type
                self.obstacle_shapes[cr_id] = shape
                self.obstacle_states[time_step][cr_id] = state
            elif self._id_mapper.has_sumo2cr(ped_id, IdDomain.PEDESTRIAN):
                # get obstacle vehicle state
                cr_id = self._id_mapper.sumo2cr(ped_id, IdDomain.PEDESTRIAN)
                self.obstacle_states[time_step][cr_id] = state

    def _get_veh_shape(self, vehicledomain, sumo_id: SumoId):
        return Rectangle(
            vehicledomain.getLength(sumo_id), vehicledomain.getWidth(sumo_id)
        )
    ...
    def _set_veh_params(
        self, domain, veh_id: int, vehicle_class: ObstacleType
    ) -> Rectangle:
        """
        ...
        def sample(val: Union[Interval, float]) -> float:
            if isinstance(val, Interval):
                assert (
                    0 <= val.start <= val.end
                ), f"All values in the interval need to be positive: {val}"
        ...
        try:
            ...
            _logger.warning(f"vehicle_class: {vehicle_class} is invalid")
            raise e
        try:
    ...
    def _extract_current_state(
        self, domain, veh_id: str, vehicle_state: Dict[int, any], id_domain: IdDomain
    ) -> Union[None, State]:
        """
        ...
        """
        unique_id = id_domain.sumo_prefix() + veh_id
        position = np.array(vehicle_state[traci.constants.VAR_POSITION])
        velocity = math.sqrt(
            vehicle_state[traci.constants.VAR_SPEED] ** 2
            + vehicle_state[traci.constants.VAR_SPEED_LAT] ** 2
        )

        cr_id = self._id_mapper.sumo2cr(veh_id, id_domain)
    ...
    def _get_cr_obstacles_at_time(
        self, time_step: int, add_ego: bool = False, start_0: bool = False
        ...
        for veh_id, state in vehicle_dict.items():
            ...
            if self.conf.add_lanelets_to_dyn_obstacles:
                center_lanelets = {
                    lanelet_id
                    for lanelet_ids in self.scenarios.lanelet_network.find_lanelet_by_position(
                        [state.position]
                    )
            ...
            signal_state = next(
                (s for s in self.signal_states[veh_id] if s.time_step == time_step),
                None,
            )
            if start_0:
    ...
    def _get_cr_obstacles_all(self) -> List[DynamicObstacle]:
        ...
        for veh_id, time_dict in veh_state.items():
            ...
            initial_state = state_list[
                0
            ]  # TODO writing an InitialState object to state_list[0] would be more clean
            initial_state = InitialState(
            ...
            assert (
                self.conf.video_start != self.conf.video_end
            ), "Simulation start time and end time are the same. Please set simulation interval."

            if len(state_list) > 4:
                obstacle_trajectory = Trajectory(
                    state_list[0].time_step, state_list[0:]
                )
                obstacle_prediction = TrajectoryPrediction(
                    obstacle_trajectory, obstacle_shape
                )
                center_lanelets = None
                shape_lanelets = None
                if self.conf.add_lanelets_to_dyn_obstacles:
                    center_lanelets = {
                        lanelet_id
                        for lanelet_ids in self.cr_scenario.lanelet_network.find_lanelet_by_position(
                ...
                dynamic_obstacle = DynamicObstacle(
                    ...
                    initial_center_lanelet_ids=center_lanelets
                    if center_lanelets
                    else None,
                    initial_shape_lanelet_ids=shape_lanelets
                    if shape_lanelets
                    else None,
                    initial_signal_state=signal_states[0] if signal_states else None,
                    signal_series=signal_states[1:] if signal_states else None,
    ...
    def get_ego_obstacles(
        self, time_step: Union[int, None] = None
    ) -> List[DynamicObstacle]:
        """
    ...
    def _send_ego_vehicles(
        self, ego_vehicles: Dict[int, EgoVehicle], delta_step: int = 0
    ) -> None:
        """
    ...
    def check_lanelets_future_change(
        self, current_state: State, planned_traj: List[State]
    ) -> Tuple[str, int]:
        """
        ...
        current_lanelets_ids = lanelet_network.find_lanelet_by_position(
            [current_position]
        )[0]
        current_lanelets = [
            lanelet_network.find_lanelet_by_id(id) for id in current_lanelets_ids
        ]
        ...
        _logger.debug("current lanelets: " + str(current_lanelets))
        _logger.debug("lc_status=" + lc_status)
        _logger.debug("lc_duration=" + str(lc_duration))
        return lc_status, lc_duration

    def _check_lc_start(self, ego_id: SumoId, lc_future_status: str) -> str:
        """
        ...
        lateral_position = self.vehicledomain.getLateralLanePosition(
            self._id_mapper.cr2sumo(ego_id, IdDomain.EGO_VEHICLE)
        )

        if lc_future_status == "NO_LC" or id not in self.lateral_position_buffer:
            lc_status = "NO_LC"
        elif (
            lc_future_status == "RIGHT_LC"
            and self.lateral_position_buffer[id]
            > self.conf.lane_change_tol + lateral_position
        ):
            lc_status = "RIGHT_LC_STARTED"
        elif (
            lc_future_status == "LEFT_LC"
            and self.lateral_position_buffer[id]
            < -self.conf.lane_change_tol + lateral_position
        ):
        ...
        _logger.debug("LC current status: " + lc_status)

        self.lateral_position_buffer[id] = lateral_position

        return lc_status

    def _consistency_protection(self, ego_id: SumoId, current_state: State) -> str:
        """
        ...
        pos_sumo = self.vehicledomain.getPosition(
            self._id_mapper.cr2sumo(ego_id, IdDomain.EGO_VEHICLE)
        )
        ...
        _logger.debug(
            "SUMO X: " + str(pos_sumo[0]) + " **** SUMO Y: " + str(pos_sumo[1])
        )
        _logger.debug("TRAJ X: " + str(pos_cr[0]) + " **** TRAJ Y: " + str(pos_cr[1]))
        _logger.debug("Error Value: " + str(dist_error))
        _logger.debug("Error Status: " + cons_error)

        if self._lc_inaction == 0:
            cons_error = "CONSISTENCY_NO_ERROR"
        return cons_error

    def _check_sync_mechanism(
        self, lc_status: str, ego_id: CommonRoadId, current_state: State
    ) -> str:
        """
        ...
        _logger.debug("Sync Mechanism is: " + retval)
        _logger.debug("Lane change performed since " + str(self._lc_counter))
        return retval

    def _forward_info2sumo(
        self,
        planned_state: State,
        sync_mechanism: str,
        lc_duration: int,
        ...
        :param ego_id: id of the ego vehicle
        """
        id_sumo = self._id_mapper.cr2sumo(ego_id, IdDomain.EGO_VEHICLE)

        if sync_mechanism == "SYNC_MOVE_XY":
            len_half = 0.5 * self.ego_vehicles[ego_id].length
            position_sumo = [0, 0]
            position_sumo[0] = planned_state.position[0] + len_half * math.cos(
                planned_state.orientation
            )
            position_sumo[1] = planned_state.position[1] + len_half * math.sin(
                planned_state.orientation
            )
            sumo_angle = 90 - math.degrees(planned_state.orientation)

            # The API of traci and libsumo use different naming for the lane parameter
            # which makes the use of named parameters infeasabile
            self.vehicledomain.moveToXY(
                id_sumo,  # vehID
                "dummy",  # edgeID
                -1,  # lane
    ...
    def is_in_field_of_view(self, position: np.ndarray, veh_id: SumoId):
        """
        ...
        """
        if (
            len(self.ego_vehicles) == 0
            or self.conf.field_of_view is None
            or self._id_mapper.has_sumo2cr(veh_id, IdDomain.VEHICLE)
            or self._id_mapper.has_sumo2cr(veh_id, IdDomain.PEDESTRIAN)
        ):
            return True
        ...
        for ego_vehicle in self.ego_vehicles.values():
            try:
                if (
                    np.linalg.norm(
                        ego_vehicle.get_state_at_timestep(
                            self.current_time_step
                        ).position
                        - position,
                        ord=np.inf,
                    )
                    < self.conf.field_of_view
                ):
                    return True
            except KeyError:
    ...
    @staticmethod
    def sort_ego_first(vehicle_ids: List[SumoId]):
        vehicle_ids = list(vehicle_ids)
        for i in range(len(vehicle_ids)):
            # TODO: check with IdDomain prefix
            if vehicle_ids[i].startswith(EGO_ID_START):
                vehicle_ids.insert(0, vehicle_ids.pop(i))
        return vehicle_ids

    def __enter__(self):
        # REFACTOR: peform initialization here to make caller code cleaner
        # self.initialize()
        return self
